#   Build Analyzer v1.0 (CH32V203C8T6)
#   Автор: Волков Олег
#   Дата создания скрипта: 17.04.2025
...
$sections = @('.init', '.vector', '.text', '.fini', '.preinit_array', '.init_array', '.fini_array ', '.ctors', '.dtors ', '.dalign', '.dlalign', '.data', '.bss', '.stack')
...
[int]$flashUsed = $sectionSizes['.init'] + $sectionSizes['.vector'] + $sectionSizes['.text'] + $sectionSizes['.fini'] + $sectionSizes['.preinit_array'] + $sectionSizes['.init_array'] + $sectionSizes['.fini_array'] + $sectionSizes['.ctors'] + $sectionSizes['.dtors'] + $sectionSizes['.dalign'] + $sectionSizes['.dlalign']+ $sectionSizes['.data']+ $sectionSizes['.bss']
